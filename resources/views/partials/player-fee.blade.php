    ...
    <tbody>
      @foreach($players as $player)
        <tr>
          ...
          @endforeach
          <td style="color: red" id='player{{$player->id}}'>${{$player->total}}</td>
        </tr>
      @endforeach

      @if( $staffs->count() == 0 )
        <tr>
          <td><B>Total team balance</B></td>
          @foreach($fees as $fee)
            <td style="color: red" id='pfee{{$fee->id}}'>${{$fee->total}}</td>
          @endforeach
          <td style="color: red" id='pteam-total'>${{$total}}</td>
        </tr>
      @endif
    </tbody>
    ...
    <tbody>
      @foreach($staffs as $staff)
        <tr>
          ...
          @endforeach
          <td style="color: red" id='nplayer{{$staff->id}}'>${{$staff->total}}</td>
        </tr>
      @endforeach
      ...
      <tr>
        <td><B>Total team balance</B></td>
        @foreach($fees as $fee)
          <td style="color: red" id='npfee{{$fee->id}}'>${{$fee->total}}</td>
        @endforeach
        <td style="color: red" id='npteam-total'>${{$total}}</td>
      </tr>
    </tbody>
